...
1. git 환경 설정하기 : git config 명령을 사용
   ...
   - git config --global user.email "메일 주소"
     ...
     !! 프로젝트마다 다른 이름을 사용할 거라면 --global 옵션 사용을 하지 않고 각 프로젝트마다 git 환경 설정을 해야 한다.
...
- vim : 리눅스의 기본 텍스트 편집기를 사용하게 하는 명령어
  ...
  ```js
   : wq
   // 편집하던 문서를 저장하고 종료한다 (뒤에 파일명을 지정해서 지정한 파일 이름으로 저장할 수 있다)
  ```
...
- git restore : 작업 트리에서 수정한 파일을 저장소에 있는 가장 최신 버전 파일로 되돌릴 때 사용하는 명령어
  ...
  // 스테이지에 있는 test.txt 파일을 다시 내린다
  ```
  ```js
  $ git restore --staged
  // 스테이지에 있는 모든 파일을 다시 내린다
  ```
...
- 원하는 작업을 마친후에 문제가 없을 시에 main 브랜치에 병합한다
  ...
  1. branch1 브랜치에서 수정한 부분을 branch2 브랜치에서 수정했고 branch1 브랜치로 병합한 경우, 병합한 이후 그 부분을 적절히 수정해주고 다시 커밋한다</small>
